...
def plot_graph():
    df = pd.read_csv("/var/LearnStreamlit/assets/data.csv")
...
def main():
    ...
    st.title("")
    # -------------------Porject-Banner--------------------
    image = Image.open('/var/LearnStreamlit/images/b1.png')
    st.image(image) 
    st.divider()
    ...
    server_folder_path = "/var/LearnStreamlit/uploaded"  # Replace with the desired folder path
    ...
    if uploaded_files:
        ...
        # copy_uploaded_files([uploaded_files], "/var/LearnStreamlit/analysis/preprocessing/csv")
        switch_page("config")
